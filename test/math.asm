...
SIS01:
	JNS	SIS05		;NOT INTEGER PROCEED
        ; THIS IS NEVER REACHED, ONLY IF TYPE WASN'T S.P.
...
SES00:	MOV	SI,BX		;WILL NEED FOR LATER
	...
	MOV	CX,WORD PTR [FAC]       ;(CH)=sign:(CL)=exp
	MOV	AL,BL		;FETCH (BXDX) SIGN BYTE
        ; This was missing, destroying mantissa in FAC
...
;MULTIPLY.
;***************************************************************
	MOV	CL,BYTE PTR [FAC+1]	;(CL)=high mantissa
	XOR	CH,CH		;(CX) CONTAINS HIGH MANTISSA BITS
	MOV	AX,WORD PTR [FACLO]	;(AX) CONTAINS LOW MANTISSA BITS OF FAC
        XCHG    AL,AH
